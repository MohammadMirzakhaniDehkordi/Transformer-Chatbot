...
for i in range(len(dialogs)-1):
    if dialogs[i+1]["reply_to"] == dialogs[i]["line_id"]:
        q=dialogs[i]["text"].lower(); a=dialogs[i+1]["text"].lower()
...
class TransformerChat(nn.Module):
    ...
    def forward(self,src,tgt):
        S,B=src.shape
        src_emb=self.embed(src)+self.pos(torch.arange(S).unsqueeze(1).to(src.device))
        mem=self.enc(src_emb)
        T,_=tgt.shape
        tgt_emb=self.embed(tgt)+self.pos(torch.arange(T).unsqueeze(1).to(tgt.device))
...
def train(epochs=5):
    ...
    for e in range(epochs):
        ...
        for i in range(0,len(X),32):
            ...
            optimizer.step()
            total+=loss.item()
        print(f"Epoch {e+1}/{epochs} - Loss: {total:.2f}")
